...
<div class="flex md:flex-row flex-col px-4 mt-10 md:p-0 gap-4">
    ...
    <div class="bg-white rounded-lg p-4 items-center border ">
        ...
        <table class="table-auto">
            ...
            <tr>
                <td class="font-semibold text-gray-600">MOTORSPORTS FOLLOWED
    </div>
    ...
    <tr>
        <td class="font-semibold text-gray-600">DRIVER SUPPORTED
</div>
...
<tr>
    ...
    <td class=" font-bold text-gray-800 px-4 py-1">
        @for($i=0;$i<count($userGames);$i++) {{$userGames[$i]}}@if($i<count($userGames)-1), @endif @endfor </td>
</tr>
@endif
</table>
</div>
<div class="bg-gray-800 rounded-lg p-4 border">
    ...
    <div class="flex w-64 flex-wrap font-semibold text-sm">
        @if(is_array($roles))
        @for ($i= 0; $i < count($roles); $i++) @php $color=str_pad($roles[$i]['color'], 6, "0" , STR_PAD_LEFT); @endphp <div class="px-1 border rounded-full mr-1 mb-1 border-600 text-gray-300" style="border-color:#{{$color}};"><i class="fas fa-circle mr-1 text-500" style="color:#{{$color}}"></i>{{$roles[$i]['name']}} </div>
    @endfor
    @else
    <!-- {{$roles}} -->
    @endif
